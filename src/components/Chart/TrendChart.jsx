...
const TrendChart = ({ 
  ...
  const handleMouseMove = (state) => {
    if (state && state.activeLabel !== undefined) {
      const hoveredHour = Number(state.activeLabel);
      if (hoveredHour !== selectedHour) {
        onSliderChange?.(hoveredHour);
      }
    }
  };
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 40, right: 30, left: 20, bottom: 40 }} onMouseMove={handleMouseMove}>
            ...
            <YAxis 
              ...
              tickFormatter={(val) => val.toLocaleString('es-ES', { minimumFractionDigits: 2 })}
            >
